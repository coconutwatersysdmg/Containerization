...
class PalletPacker:
    ...
    def _place_homogeneous_grid(
        ...
        for idx, item in enumerate(group):
            ...
            if point['z'] > 1e-9 and direct_support_ratio(point, dims, current) + 1e-9 < 0.8:
                return []
    ...
    def _place_topup_rows(
        ...
        for item in ordered:
            ...
            if direct_support_ratio(point, dims, current) + 1e-9 < 0.8:
                x += dims['length']
                row_width = max(row_width, dims['width'])
                continue
            if not passes_same_size_heavier_below_constraint(
            ...
            for item in ordered:
                ...
                for candidate in sorted(
                    ...
                    if (
                        ...
                        continue
                    if not self._is_valid_packed(trial, pallet_dims):
                        if diag is not None:
