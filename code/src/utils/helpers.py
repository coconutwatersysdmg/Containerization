...
def passes_small_box_below_constraint(point, dims, item_volume_value, placed_boxes):
    """小箱子约束：在 point 放置后，下方若有更小体积的箱子且 XY 投影
    与本箱重叠，则违例（小箱不能压大箱）。

    Args:
        point: 候选位置 {'x', 'y', 'z'}
        dims: 候选尺寸 {'length', 'width'}（用于 XY 投影）
        item_volume_value: 候选箱子的体积
        placed_boxes: 已放置箱子列表

    Returns:
        True 表示通过；False 表示违例。
    """
    if point['z'] <= 1e-9:
        return True
    ...
    for placed_box in placed_boxes:
        ...
        if item_volume_value < placed_volume - 1e-9:
            return False
    return True
