...
def fast_rescue_failed_pallets_by_topup(
    ...
    for receiver in receivers:
        ...
        refresh_pallet_stability_status(receiver, pallet_dims)
        ...
        for donor in type_plans:
            if id(donor) in changed_donors:
                refresh_pallet_stability_status(donor, pallet_dims)

        changed_plans = [receiver] + [
            donor for donor, _ in old_donor_items.values()
        ]
        if not all(
            validate_pallet_constraints(plan, pallet_dims)["is_valid"]
