...
class BeamSearchPacker:
    ...
    def _generate_feasible_candidates(
        ...
        item_volume = raw_length * raw_width * raw_height
        ...
        for z_level in sorted_z_levels:
            ...
            for point in grouped_by_z[z_level]:
                ...
                # 尺寸顺序检查
                if not self.placement_validator.satisfies_size_order(
                    point, dims, item_volume, placed_boxes
                ):
                    continue

                if not self.placement_validator.satisfies_stacking_order(
