...
from src.utils.helpers import (
    ...
    passes_small_box_below_constraint,
...
def _make_reachability_checker(pallet_dims: Dict[str, float]) -> SuctionPlanner:
    """构造一个机械臂可达性检查器，与原算法默认参数一致。"""
...
def build_direct_layer_packing_solution(
    ...
    reachability_checker = _make_reachability_checker(pallet_dims)
    ...
    def _try_pack_pool(pool):
        ...
        for (length, width, height, mpm), group in grouped_entries:
            ...
            for idx, item in enumerate(ordered_group):
                ...
                if not passes_small_box_below_constraint(
                    point,
                    dims,
                    effective_length * effective_width * effective_height,
                    placed,
                ):
                    return []
                ...
                if not passes_box_gap_constraint(
                    point, dims, raw_dims, placed, max_gap=MAX_BOX_GAP_MM
                ):
                    return []
                if (
                    point['z'] > 0
...
def build_centered_single_box_solution(
    ...
    for item in ordered_items:
        ...
        if validate_center_of_mass(
            {'packed_items': [item_copy]}, pallet_dims
        ).get('is_stable', False):
            return [item_copy]
