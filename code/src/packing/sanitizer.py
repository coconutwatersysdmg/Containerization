        ...
        if not unstable_items:
            ...
            if pallet_dims:
                ...
                if not com_result.get('is_stable', False):
                    victim = _select_com_victim(kept_items, pallet_dims)
                    if victim is None:
                        break
                    kept_items = [
                        item for item in kept_items
...
def _select_com_victim(
    kept_items: List[Dict], pallet_dims: Dict
) -> Optional[Dict]:
    """挑出对重心偏移贡献最大的可拿走箱子。"""
    ...
    tol_x = float(pallet_dims['length']) / 3.0
    tol_y = float(pallet_dims['width']) / 3.0
