...
class PlacementValidator:
    ...
    def satisfies_size_order(
        ...
        Examples:
            ...
            ...     'is_small_box': True
            ... }]
            >>> validator.satisfies_size_order(point, dims, item_volume, placed)
            True
        """
        # 地面上的箱子不受此约束
        if point['z'] <= 1e-9:
        ...
        from ..geometry.overlap import has_positive_xy_overlap

        for placed_box in placed_boxes:
            # 只检查标记为小箱子的已放置箱子
            if not placed_box.get('is_small_box', False):
                continue
            ...
            if placed_top_z > point['z'] + 1e-6:
                continue

            # 检查XY平面是否有重叠
            if not has_positive_xy_overlap(point, dims, placed_box):
                continue

            # 如果候选箱子体积小于下方的小箱子，违反约束
            placed_volume = (
                placed_box['length'] *
                placed_box['width'] *
                placed_box['height']
            )
            if item_volume < placed_volume - 1e-9:
                return False
