...
def test_rescue_optimizer():
    ...
    # 测试_gap_of
    pallet = {'mpm_gap': 10.5}
    gap = optimizer._gap_of(pallet)
    print(f"MPM缺口: {gap}")
    assert gap == 10.5

    # 测试_receiver_tier
    tier = optimizer._receiver_tier(5.0)
    print(f"缺口5.0的层级: {tier}")
    assert tier == "near"

    tier = optimizer._receiver_tier(15.0)
    print(f"缺口15.0的层级: {tier}")
    assert tier == "mid"

    tier = optimizer._receiver_tier(30.0)
    print(f"缺口30.0的层级: {tier}")
    assert tier == "deep"

    # 测试_top_layer_items
    pallet = {
        'packed_items': [
            {'position': {'x': 0, 'y': 0, 'z': 0}, 'height': 100},
            {'position': {'x': 0, 'y': 0, 'z': 100}, 'height': 100}
        ]
    }
    top_items = optimizer._top_layer_items(pallet)
    print(f"顶层箱子数量: {len(top_items)}")
    ...
    class StubPacker:
        ...
        def pack(self, items, **kwargs):
            ...
            for idx, item in enumerate(items):
                box = dict(item)
                box['position'] = {'x': idx * 100, 'y': 0, 'z': 0}
                packed.append(box)
            return packed, []
